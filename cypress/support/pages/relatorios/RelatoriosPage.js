...
class RelatoriosPage {
    ...
    /**
     * Fluxo alternativo (A) - acessa a página de relatórios gerais e abre o relatório
     * via modal de busca (sem navegar diretamente pela rota do relatório).
     *
     * Observação: este fluxo normalmente abre um painel/drawer do relatório dentro de
     * /relatorios-gerais, ao invés de trocar a URL para /relatorio-v2/*.
     */
    acessarRelatorioViaRelatoriosGerais(nomeRelatorio) {
        cy.visit(RelatoriosGeraisLocators.paginaRelatoriosGerais);

        cy.get(RelatoriosGeraisLocators.campoBuscaPagina, { timeout: 10000 })
            .should('be.visible')
            .type(nomeRelatorio, { force: true })

        cy.wait(3000);
        // console.log('Esperando 3 segundos');
        cy.get(RelatoriosGeraisLocators.campoBuscaModal, { timeout: 15000 })
            .click({ force: true })
    ...
    acessarRelatorio(linkLocator) {
        ...
        cy.get(linkLocator, { timeout: 10000 }).first().should('exist').then(($card) => {
